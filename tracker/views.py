...
from .forms import BudgetForm, ExpenseForm, IncomeForm, LoginForm, SignUpForm
from .models import Account, Budget, Category, Transaction, UserProfile
...
def home_expense(request):
    ...
    try:
        ...
        print(
            f"All budgets for user {user.username}: {list(all_budgets.values('name', 'amount', 'period', 'start_date', 'end_date'))}"
        )
        ...
        if current_budget:
            ...
            budget_used_percentage = (
                (total_expenses / monthly_budget * 100) if monthly_budget > 0 else 0
            )
            print(
                f"Found current budget: {current_budget.name}, Amount: {current_budget.amount}"
            )
        else:
            monthly_budget = 0
...
def delete_expense(request, expense_id):
    try:
        expense = Transaction.objects.get(
            pk=expense_id, user=request.user, transaction_type="expense"
        )
        expense.delete()
...
def export_expenses(request):
    ...
    for expense in expenses:
        ...
        writer.writerow(
            [category_name, expense.amount, expense.date, expense.description]
        )
...
def update_budget(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print(
                f"Budget saved: {obj.name}, Amount: {obj.amount}, Period: {obj.period}"
            )
