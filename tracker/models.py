...
class Budget(TimestampedModel):
    ...
    def __str__(self):
        return (
            f"{self.name} - {self.amount} {self.currency} ({self.get_period_display()})"
        )
...
class Rule(TimestampedModel):
    ...
    def matches_transaction(self, transaction):
        ...
        if self.rule_type == "contains":
            return (
                self.pattern.lower() in transaction.description.lower()
                or self.pattern.lower() in transaction.merchant.lower()
            )
            ...
            try:
                return re.search(
                    self.pattern, transaction.description, re.IGNORECASE
                ) or re.search(self.pattern, transaction.merchant, re.IGNORECASE)
            except re.error:
                return False
        elif self.rule_type == "exact":
            return (
                self.pattern.lower() == transaction.description.lower()
                or self.pattern.lower() == transaction.merchant.lower()
            )
