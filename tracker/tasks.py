...
from .models import (
    Transaction, Import, Budget, UserProfile, Category, 
    Account, Attachment, Rule
)
        ...
        for import_record in pending_imports:
            try:
                if import_record.source == 'csv':
                    ...
                    import_record.error_log.append(f"Unknown source: {import_record.source}")
                    import_record.save()
    ...
    try:
        ...
        for profile in users_with_reports:
            try:
                ...
                logger.error(f"Error generating report for user {profile.user.id}: {str(e)}")
...
def generate_user_weekly_report(self, user_id):
    ...
    try:
        ...
        report_data['net_flow'] = report_data['total_income'] - report_data['total_expenses']
    ...
    try:
        ...
        for transaction in recurring_transactions:
            try:
                ...
                logger.info(f"Created recurring transaction {new_transaction.id} for user {transaction.user.id}")
    ...
    try:
        ...
        for rule in rules:
            if rule.matches_transaction(transaction):
                ...
                transaction.save()
                logger.info(f"Applied rule '{rule.name}' to transaction {transaction_id}")
                break
                
    except Transaction.DoesNotExist:
        logger.error(f"Transaction {transaction_id} not found")
    except Exception as e:
        logger.error(f"Error applying categorization rules to transaction {transaction_id}: {str(e)}")
        raise self.retry(countdown=60, exc=e)
...
def export_user_data(self, user_id, format_type='csv'):
    ...
    try:
        ...
        if format_type == 'csv':
            ...
            # Write transactions
            writer.writerow(['Type', 'Date', 'Amount', 'Currency', 'Description', 'Category', 'Account', 'Merchant'])
