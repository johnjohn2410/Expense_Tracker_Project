...
from tracker.models import Category, Account, UserProfile
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        default_accounts = [
            ...
            {'name': 'Checking Account', 'account_type': 'checking', 'balance': 0, 'currency': 'USD'},
            {'name': 'Savings Account', 'account_type': 'savings', 'balance': 0, 'currency': 'USD'},
            {'name': 'Credit Card', 'account_type': 'credit', 'balance': 0, 'currency': 'USD'},
        ]
        ...
        for user in User.objects.all():
            for acc_data in default_accounts:
                ...
                if created:
                    self.stdout.write(f'Created account {account.name} for user {user.username}')
